function [shortestPaths, totalCosts] = kShortestPath_QNetwork(netCostMatrix, source, destination, k_paths)
...
% This function calls a function dijkstra_QNetwork(), which is modified to account for quantum routing metric-fidelity 
...
if source > size(netCostMatrix,1) || destination > size(netCostMatrix,1)
    ...
    %---------------------INITIALIZATION---------------------
    k=1;
    [path cost] = dijkstra_QNetwork(netCostMatrix, source, destination);
    ...
    if isempty(path)
        ...
        %***********************
        % K = 1 is the shortest path returned by dijkstra_QNetwork():
        ...
        while (k < k_paths   &&   size_X ~= 0 )
            ...
            for index_dev_vertex= w_index_in_path: length(P_) - 1   %index_dev_vertex is index in P_ of deviation vertex
                ...
                for j = 1: length(SP_sameSubPath)
                    ...
                    %%%%%%added by Vini to make 'temp_netCostMatrix'symmetric %%%%%%%%%%%%%
                    temp_netCostMatrix(next,v_)=inf; 
                end
                %------
...
%                 cost_sub_P=0;
                cost_sub_P=1/1;             %fidelity_init = 1 (max possible), distance = 1/fidelity_init = 1/1 (min possible)
                for i = 1: length(sub_P)-1
%                     cost_sub_P = cost_sub_P + netCostMatrix(sub_P(i),sub_P(i+1));
                    cost_sub_P = 1/CascadedFidelity( (1/cost_sub_P),(1/netCostMatrix(sub_P(i),sub_P(i+1))) );
                end

                %call dijkstra_QNetwork between deviation vertex to destination node    
                [dev_p c] = dijkstra_QNetwork(temp_netCostMatrix, P_(index_dev_vertex), destination);
                if ~isempty(dev_p)
                    path_number = path_number + 1;
                    P{path_number,1} = [sub_P(1:end-1) dev_p] ;  %concatenate sub path- to -vertex -to- destination
%                     P{path_number,2} =  cost_sub_P + c ;
                    P{path_number,2} =  1/CascadedFidelity( (1/cost_sub_P),(1/c) );
